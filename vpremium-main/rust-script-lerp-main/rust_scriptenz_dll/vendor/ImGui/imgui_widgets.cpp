...
void ImGui::Bullet()
{
    ...
    ImU32 text_col = GetColorU32(ImVec4(1, 0, 0, 1));
...
bool ImGui::ButtonMinusEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x + 0.f, label_size.y + style.FramePadding.y + 0.f);
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, colBorder, 0.f, 15, 0);

    RenderText(ImVec2(bb.Max.x + style.ItemInnerSpacing.x - 12, bb.Min.y + 1), label);
...
bool ImGui::ButtonPosEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderText(ImVec2(bb.Max.x + style.ItemInnerSpacing.x - 13, bb.Min.y + 2), label);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x + 5.f, label_size.y + style.FramePadding.y + 14);
...
bool ImGui::ButtonExI(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x + 20.0f, label_size.y + style.FramePadding.y + 3);
    ...
    //RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    const auto colA = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_WindowBg)); // ImGuiCol_WindowBg
    const auto colB = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_WindowBg));
    window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, colA, colA, colB, colB);
    const auto colBorder = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_WindowBg));
    window->DrawList->AddRect(bb.Min, bb.Max, colBorder, 0.f, 15, 0);
...
{
    return ButtonPosEx(label, size_arg, ImGuiButtonFlags_None);
}

bool ImGui::ButtonMinus(const char* label, const ImVec2& size_arg)
{
    return ButtonMinusEx(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::ButtonI(const char* label, const ImVec2& size_arg)
{
    return ButtonExI(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    RenderFrame(bb.Min, bb.Max, GetColorU32(ImVec4{ 30.f / 255.f, 30.f / 255.f, 30.f / 255.f, 1.0f }), true, 2.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    {
        float t_anim = ImSaturate(g.LastActiveIdTimer / ANIM_SPEED); // boolean true dondurdugunda 0'dan 1'e += 0.30f artarak gidiyor.
    ...
    {
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(41.f / 255.f, 41.f / 255.f, 41.f / 255.f, 1.f), ImVec4(126.f / 255.f, 131.f / 255.f, 219.f / 255.f, 1.f), t));
        checkbg = ImGui::GetColorU32(ImLerp(ImVec4(0.17f, 0.17f, 0.17f, 1.0f), ImVec4(0.17f, 0.17f, 0.17f, 1.0f), t));
    }
    ...
    RenderFrame(check_bb.Min, check_bb.Max, col_bg, true, 3);
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);

    if (*v)
    {
        const float pad = ImMax(t, IM_FLOOR(square_sz / 6.0f));
        window->DrawList->AddCircleFilled(check_bb.GetCenter(), 12.f + (t * 4), GetColorU32(ImVec4(126.f / 255.f, 131.f / 255.f, 219.f / 255.f, 1.f - t)), 32);
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad * t, pad), checkbg, square_sz - pad * (t * 2.00f));
    }
    else
        window->DrawList->AddRect(check_bb.Min, ImVec2(check_bb.Max.x, check_bb.Max.y), GetColorU32(ImVec4(1.00f, 1.00f, 1.00f, 1.0f)), 3); // outline rect
...
bool TopButtonEx(const char* label, bool inuse, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (inuse)
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 3), ImVec2(bb.Max.x, bb.Max.y), ImGui::GetColorU32(ImVec4{ 126.f / 255.f, 131.f / 255.f, 219.f / 255.f, 1.f }));
    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    ImGui::PushStyleColor(ImGuiCol_Text, inuse ? ImVec4{ 1.f,1.f,1.f,1.f } : ImVec4{ 0.7f,0.7f ,0.7f ,1.f });
    ImGui::RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding - ImVec2(0, 15), label, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopStyleColor();
    // Automatically close popups
    //if (pressed && !(flags & ImGuiButtonFlags_DontClosePopups) && (window->Flags & ImGuiWindowFlags_Popup))
    //    CloseCurrentPopup();
...
bool ImGui::TopButton(const char* label, bool inuse, const ImVec2& size_arg)
{
    return TopButtonEx(label, inuse, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    auto color_grab = GetColorU32(ImVec4(0.42, 0.06, 0.93, 1.0));
    auto color_full = GetColorU32(ImVec4(0.76, 0.62, 0.973, 1.0));
    auto color_circle = GetColorU32(ImVec4(0.42, 0.06, 0.93, 1.0));
    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    ...
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 4), GetColorU32(ImVec4(30.f / 255.f, 30.f / 255.f, 30.f / 255.f, 1.0f)), true, 2); //Slider Tam uzunlugu

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 4), ImVec2(grab_bb.Max.x + 1, frame_bb.Max.y - 4), GetColorU32(ImVec4(126.f / 255.f, 131.f / 255.f, 219.f / 255.f, 1.f)), 2, ImDrawCornerFlags_BotLeft | ImDrawCornerFlags_TopLeft); //Slider in secili olan yere kadar uzunlugu (Mavi olan)

    window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - 2, grab_bb.Min.y - 2), ImVec2(grab_bb.Max.x + 2, grab_bb.Max.y + 2), GetColorU32(ImVec4(1.00f, 1.00f, 1.00f, 1.00f)), 12); // Secili olan yerin sonundaki beyaz secme yeri
    float xheight = ImGui::GetFrameHeight();
    float xradius = xheight * 0.50f;
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - CalcTextSize(label, NULL, true).y - 4), label);

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags);
    return value_changed;
}

bool ImGui::SliderScalarAlt(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_HeaderActive : g.HoveredId == id ? ImGuiCol_HeaderActive : ImGuiCol_HeaderActive);
    RenderNavHighlight(frame_bb, id);
    const auto colA = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_HeaderActive));
    const auto colB = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_HeaderActive));
    ...
    const auto colBorder = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border));
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, colBorder, 0.f, 0, 0);
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    ...
    ///
    /// render text
    ImVec2 value_text_size = CalcTextSize(value_buf, NULL, true);

    RenderTextClipped(ImVec2(frame_bb.Min.x - 163, frame_bb.Max.y - 25), ImVec2(frame_bb.Max.x + 267 - value_text_size.x, frame_bb.Max.y - 15), value_buf, value_buf_end, NULL, ImVec2(0.51f, 0.5f));
...
bool ImGui::SliderFloatAlt(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalarAlt(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::SliderIntAlt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalarAlt(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 16.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
