...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, colBorder, 0.f, 15, 1.5f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
   ...
   // RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
    RenderFrame({ check_bb.Min.x + 2, check_bb.Min.y + 2 }, { check_bb.Max.x - 2, check_bb.Max.y - 2 }, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
    ...
    if (mixed_value)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad.x + 2, pad.y + 2), check_bb.Max - ImVec2(pad.x + 2, pad.y + 2), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
    }
    else if (*v)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        //RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad + 1, pad + 1), check_bb.Max - ImVec2(pad + 1, pad + 1), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
    }
    else {
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 3), GetColorU32(ImGuiCol_FrameBg), true, 3);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        ...
        window->DrawList->AddRectFilled((ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y)), (ImVec2(grab_bb.Max.x, grab_bb.Min.y)), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 5);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    //RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    RenderText({ frame_bb.Max.x - 20, frame_bb.Min.y + 3 }, value_buf, value_buf_end, NULL);

    if (label_size.x > 0.0f)
       // RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
        RenderText(ImVec2(frame_bb.Min.x + 3, frame_bb.Min.y + 3), label);
