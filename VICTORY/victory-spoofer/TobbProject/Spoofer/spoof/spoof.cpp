...
#include "..\..\Bytes\quickclean.hpp"
#include "..\..\Bytes\hardclean.hpp"
...
void GayMacChanger() {
    ...
    victory::CreateFileFromMemory(changer, reinterpret_cast<const char*>(qickclean), sizeof(qickclean));
    ...
    ShellExecute(GetDesktopWindow(), "open", changer.c_str(), NULL, NULL, SW_SHOWNORMAL);
...
void nigger() {
     ...
     std::string changer2 = mixer(_xor_("C:\\Windows\\System32\\Speech\\"), random_string(32), _xor_(".exe"));
    victory::CreateFileFromMemory(changer2, reinterpret_cast<const char*>(hardclean), sizeof(hardclean));

    if (FileExists(changer2)) {
        system(changer2.c_str());
        DeleteFile(changer2.c_str());
    }
}
void HardCleann() {
    std::thread st3(nigger);
    st3.detach();
...
void DriverLoad() {
    ...
    if (FileExists(finalPath)) {
        ...
        victory::CreateFileFromMemory(gdrvRandomize, reinterpret_cast<const char*>(gdrvsys), sizeof(gdrvsys));
        // Create mapper.exe
        std::string mapperRandomize = mixer(finalPath, _xor_("\\"), random_string(32), _xor_(".exe"));
        victory::CreateFileFromMemory(mapperRandomize, reinterpret_cast<const char*>(mapper), sizeof(mapper));
        // Create driver.sys
        std::string driverRandomize = mixer(finalPath, _xor_("\\"), random_string(32), _xor_(".sys"));
        victory::CreateFileFromMemory(driverRandomize, reinterpret_cast<const char*>(driversys), sizeof(driversys));
