...
#define PORT 3131
#define SERVERIP "80.253.250.51"
std::string VERSION = _xor_("0.0.1");
...
bool Client::setupEncryption() {
	...
	if (civQuery.size() == successCiv.size()) {		
		// Version control
		std::string successVer = _xor_("VERSION_SUCCESS_AUTHENTICATION");
		std::string versionQuery = aes_enc->decrypt(cipher_key, iv_key,
			sendrecieve(aes_enc->encrypt(cipher_key, iv_key, mixer(_xor_("VERSION_QUERY;"), VERSION))
		));
		if (successVer.size() == versionQuery.size()) {
			// Version verified
			return true;
		}
		else {
