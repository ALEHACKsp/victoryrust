...
void Scenes::Cheat() {
    ...
    if (GetServiceStatus(_xor_("MBAMService").c_str()) == SERVICE_RUNNING) {
        printf(_xor_("Please uninstall malwarebytes, until then you will not be able to spoof.").c_str());
        Sleep(2500);
        exit(0);
    }


    const int spoofopt = MessageBoxA(0, _xor_("Would you like to spoof?").c_str(), (_xor_("Victory Loader").c_str()), MB_YESNO);
    switch (spoofopt)
    {
    case IDYES:
        printf(_xor_("\n   [+] Spoofing\n").c_str());
